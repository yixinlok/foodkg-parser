...
def print_result(results):
    ...
    for hit in range(0, len(Hitted_recipe_names)):
        print(str(hit+1) + "." + "(" + Hitted_ids[hit] + "," + "name:" + Hitted_recipe_names[hit] +
              ", ingredients " + Hitted_ingredients[hit] + ", nutritional data" + Hitted_nutri[hit] + ")")
...
def sTool(first, second):
    ...
    while(open):
        ...
        with ix.searcher() as searcher:
            ...
            while(True):
                answer = input("Do you want to search by " +
                               second + " ? (y = yes, n = no):")
                if answer == "y" or answer == "Y":
                    results = searcher.search(q, limit=10, filter=query.Term(second, input(
                        "Type in the sepcific " + second + ", (if more than one use ',' to separte)\n")))
                    break
